...
### Minimum Duration Supported
The timer’s resolution is one tick:
$$t_{\min} = \frac{1}{f_{\text{tick}}} = 1 \, \mu s.
$$
...
For long delays (rests or note holds), we use TIM15 with the same tick frequency of $(1 \, \mu s)$. The maximum delay is:
$$t_{\max} = (ARR_{\max} + 1) \times \frac{1}{f_{\text{tick}}} 
...
$\frac{220.02\ \text{Hz} - 220\ \text{Hz}}{220\ \text{Hz}} = 0.0001 = 0.01\%$
